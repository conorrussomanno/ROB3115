...
//--------------------------------------------------------------
void HUDSpinner::setup(int count, float avgRotateSpeed, float rotationVariation, ofColor baseColor, float resolution, float minRadius, float maxRadius, float minWidth, float maxWidth, bool _hasTail){
    ...
    for (int i = 0; i < count; i++){
        ...
        bool hasTail = _hasTail;
        tempUnit.setup(rotateSpeed_, color_, numSegments_, startSweep_, sweep_, innerEdge, outerEdge, hasTail);
        units.push_back(tempUnit);
    }
}

//--------------------------------------------------------------
void HUDSpinner::update(float currentAttValue, float _currentSignalQuality){
    
    if(_currentSignalQuality == 100){
        for(int i = 0; i < units.size(); i++ ){
            units[i].update(currentAttValue);
        }
    }
}
