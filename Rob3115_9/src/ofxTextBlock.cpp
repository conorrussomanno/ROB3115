...
void ofxTextBlock::drawLeftAndType(float x, float y, int wordNumber){
    ...
    if (words.size() > 0) {
        ...
        for(int l=0; l < lines.size(); l++)
        {
            ...
            for(int w=0;w < lines[l].wordsID.size(); w++)
            {
                if(counter < wordNumber){  //checks to see if the number of words that has been printed is greater than the amount that are supposed to be... for typing animation...
                    currentWordID = lines[l].wordsID[w];
