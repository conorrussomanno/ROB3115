...
bool EEG::setup(int deviceType){
    bool connection;
    if(deviceType == 1){
        connection = thinkGear.setup("/dev/tty.MindWaveMobile-DevA", 0);
    }
    if(deviceType == 2){
        connection = thinkGear.setup("/dev/tty.MindSet-DevB", 0);
    }
    if(deviceType == 3){
        connection = thinkGear.setup("/dev/tty.BrainBand-DevB", 0);
    }
    
    ofAddListener(thinkGear.attentionChangeEvent, this, &EEG::attentionListener);
    
    font.loadFont("font/Orbitron_Light.ttf", (18.0/1680)*ofGetWidth());
    distAw = 0.0;
    prevAw = 0.0;
    currAw = 0.0;
...
float EEG::getStoryAttention(){
    float storyAttTotal = 0;
    for(int i = 0; i < attValues.size(); i++){
        storyAttTotal += attValues[i];
    }
    
    if(attValues.size()>0){
        return (float)storyAttTotal/attValues.size();
    }
    else{
        return 0.0;
    }
...
void EEG::drawAttentionGraph()
{
    ...
    for(int i = 0; i < (int)chapValues.size(); i++){
        total+=chapValues[i];
    }
    total=total/(chapValues.size());
    cout << "total: " << total << endl;
    
    for(int i = 0; i < (int)chapValues.size()/2; i++){
        firstHalf+=chapValues[i];
    }
    firstHalf=firstHalf/(chapValues.size()/2);
    cout << "firstHalf: " << firstHalf << endl;
    
    
    for(int i = (int)chapValues.size()/2; i < chapValues.size(); i++){
        secondHalf+=chapValues[i];
    }
    secondHalf=secondHalf/(chapValues.size()/2);
    ...
    if(chapValues.size()>0){
        for (int i = 0; i < chapValues.size()-1; i++) {
            ofPushStyle();
            ofSetColor(0, 0, 255);
            ofEllipse(ofMap(i, 0, chapValues.size(), (50.0/1680.0)*ofGetWidth(), ofGetWidth()-(50.0/1680.0)*ofGetWidth()), ofGetHeight()/2+(100.0/1680.0)*ofGetWidth()-chapValues[i]*2, (5.0/1680.0)*ofGetWidth(), (5.0/1680.0)*ofGetWidth());
            ofSetLineWidth((5.0/1680.0)*ofGetWidth());
            ofLine(ofMap(i, 0, chapValues.size(), (50.0/1680.0)*ofGetWidth(), ofGetWidth()-(50.0/1680.0)*ofGetWidth()), ofGetHeight()/2+(100.0/1680.0)*ofGetWidth()-chapValues[i]*2, ofMap(i+1, 0, chapValues.size(), (50.0/1680.0)*ofGetWidth(), ofGetWidth()-(50.0/1680.0)*ofGetWidth()), ofGetHeight()/2+(100.0/1680.0)*ofGetWidth()-chapValues[i+1]*2);
            ofEllipse(ofMap(i+1, 0, chapValues.size(), (50.0/1680.0)*ofGetWidth(), ofGetWidth()-(50.0/1680.0)*ofGetWidth()),ofGetHeight()/2+(100.0/1680.0)*ofGetWidth()-chapValues[i+1]*2, (5.0/1680.0)*ofGetWidth(), (5.0/1680.0)*ofGetWidth());
            ofPopStyle();
        }
    }
