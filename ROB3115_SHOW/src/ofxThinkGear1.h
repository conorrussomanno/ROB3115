...
const int TG_BAUDRATE1 = 9600;

// Data format for use with TG_Connect() and TG_SetDataFormat()
const int TG_STREAM_PACKETS1 = 0;
const int TG_STREAM_5VRAW1 = 1;
const int TG_STREAM_FILE_PACKETS1 = 2;

// Data type that can be requested from TG_GetValue()
const int TG_DATA_BATTERY1 = 0;
const int TG_DATA_POOR_SIGNAL1 = 1; // 0 = good. others = poor.
const int TG_DATA_ATTENTION1 = 2; // min = 0. max = 100.
const int TG_DATA_MEDITATION1 = 3; // min = 0. max = 100.
const int TG_DATA_RAW1 = 4;
const int TG_DATA_DELTA1 = 5;
const int TG_DATA_THETA1 = 6;
const int TG_DATA_ALPHA11 = 7;
const int TG_DATA_ALPHA21 = 8;
const int TG_DATA_BETA11 = 9;
const int TG_DATA_BETA21 = 10;
const int TG_DATA_GAMMA11 = 11;
const int TG_DATA_GAMMA21 = 12;
const int TG_DATA_BLINK_STRENGTH1 = 37; // min = 0. max = 255.
...
//    void setup(string deviceName = "/dev/tty.MindSet-DevB", int _id = 0)
//    {
    ...
    void setup(string deviceName = "/dev/tty.MindWaveMobile-DevA", int _id = 0)
    {
        ...
        int conResult = TG_Connect(connectionID, deviceName.c_str(), TG_BAUDRATE1, TG_STREAM_PACKETS1);
    ...
    void update()
    {
        if(!failedConnection){
            ...
            if (numPackets > 0) 
            {
                signalQuality = TG_GetValue(connectionID, TG_DATA_POOR_SIGNAL1);

                if (signalQuality == 0.0) 
                {
                    if (TG_GetValueStatus(connectionID, TG_DATA_ATTENTION1) != 0.0)
                    {
                        float attention = TG_GetValue(connectionID, TG_DATA_ATTENTION1);
                        ofNotifyEvent(attentionChangeEvent, attention);
                    }
                    if (TG_GetValueStatus(connectionID, TG_DATA_MEDITATION1) != 0.0)
                    {
                        float meditation = TG_GetValue(connectionID, TG_DATA_MEDITATION1);
                        ofNotifyEvent(meditationChangeEvent, meditation);
                    }   
                    if (TG_GetValueStatus(connectionID, TG_DATA_BLINK_STRENGTH1) != 0.0)
                    {
                        float blink = TG_GetValue(connectionID, TG_DATA_BLINK_STRENGTH1);
